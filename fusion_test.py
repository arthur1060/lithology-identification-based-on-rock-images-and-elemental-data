...
def ele_modification(image_id, poss_max, x, y):
    ...
    poss3 =torch.randn(2,9).cuda()
    elem3 = torch.randn(2, 11).cuda()
    #print("image_id",image_id)
    #print("element",elem2[0])
    #print("poss",poss2[0])
    ...
    if torch.cuda.is_available():
        model1 = model1.cuda()
        model1.load_state_dict(torch.load("D:\\PycharmProjects\\pytorch-YOLOv4-new\\weight\\f20.9606.pkl"))
        model1.eval()

        with torch.no_grad():
            out = model1(elem2,poss2)
        _, lithology = torch.max(out, 1)
        #print("out",lithology[0])
...
def stest( eval_model, val_loader,config, device,conf_thresh = 0.4 ,nms_thresh = 0.5 ):
    ...
    for images, targets in val_loader:
        ...
        #print(model_width_height)
        #print(targets)  #[{'boxes': tensor([[1260., 1060., 2848., 2880.]]), 'labels': tensor([6]), 'image_id': tensor([50223]), 'area': tensor([8202240.]), 'iscrowd': tensor([0])},
        ...
        for j in range(config.batch):
            ...
            if len(boxes[0]) == 0:
                ...
                for i in range(len(boxes[0])):
                    ...
                    poss = boxes[0][i][-2]
                    image_id = str(image_ids[j].item())
                    poss_max = torch.tensor(boxes[0][i][-3], dtype=torch.float32)
                    x, y = int(((x1 + x2) / 2)), int(((y1 + y2) / 2))
                    #lithology = ele_modification(image_id, poss_max, x, y)
                ...
                # label = targets[j]["labels"].numpy()[0]
                # print("lith_check", label, type(label))
                # if label!=lith:
                #    print("imageID:",image_id,"pre_lith:",lith,"improved lithology:",lithology,"reallabel:",label)

                h = torch.tensor(boxes_acc, dtype=torch.float32).to(device)
                output[j] = h
        # output_nms containing b list    tar*6[x,y,x,y,poss,litho]
