...
class Yolov4s(nn.Module):
    def __init__(self, yolov4conv137weight=None, n_classes=80, inference=False):
        ...
        # head
        self.head = Yolov4Heads(output_ch, n_classes, inference)
...
def get_region_boxes(boxes_and_confs):
    ...
    for item in boxes_and_confs:
        ...
        confs_w_list.append(item[1])

    # boxes: [batch, num1 + num2 + num3, 1, 4]
    # confs: [batch, num1 + num2 + num3, num_classes]
    boxes = torch.cat(boxes_list, dim=1)
    confs_w = torch.cat(confs_w_list, dim=1)

    return [boxes,  confs_w]

class Yolov4Heads(nn.Module):
...
class YoloLayers(nn.Module):
    ...
    def forward(self, output, target=None):
        ...
        return yolo_forward_dynamics(output, self.thresh, self.num_classes, masked_anchors, len(self.anchor_mask),scale_x_y=self.scale_x_y)


def yolo_forward_dynamics(output, conf_thresh, num_classes, anchors, num_anchors, scale_x_y, only_objectness=1,
                         validation=False):
    ...
    cls_confso = cls_confs.permute(0, 1, 3, 2).reshape(output.size(0), num_anchors * output.size(2) * output.size(3),
                                                       num_classes)
    ...
    cls_confs = torch.sigmoid(cls_confso)
    ...
    confs_w = torch.cat((confs, det_confs), dim = -1)
    #print("confs_w-confs", confs_w.size() ,confs.size())
